    ...
    public class CarManagementController : AdminMainController
    {
        ...
        public ActionResult AddCarImages(FormCollection form)
        {
            try
            {
                ...
                string _carImagePath = String.Concat(new DirectoryInfo(Server.MapPath("~")), "/", String.Format(functionHelper.GetFromConfig("CarImageFolder"), carID));
        ...
        public ActionResult NewCar()
        {
            ...
            try
            {
                ...
                List<VehicleTrim> _lstVehicleTrim = new List<VehicleTrim>();
                _lstVehicleTrim = (from vtm in _ctx.VehicleTrims
                             where vtm.IsActive == true
                             && vtm.IsDeleted == false
                             select vtm).OrderBy(obj => obj.Name).ToList();
                if (_lstVehicleTrim != null)
                    ViewBag.VehicleTrimList = _lstVehicleTrim;
                ...
                ViewBag.TransmissionList = new SelectList(Enum.GetValues(typeof(Web.Helpers.DataHelper.CarTransmission))
                    .Cast<Web.Helpers.DataHelper.CarTransmission>()
                    .Select(obj => new SelectListItem { Text = obj.ToString(), Value = ((int)obj).ToString() })
                    .AsEnumerable(), "Value", "Text");

                ViewBag.ConditionList = new SelectList(Enum.GetValues(typeof(Web.Helpers.DataHelper.CarCondition))
                    .Cast<Web.Helpers.DataHelper.CarCondition>()
                    .Select(obj => new SelectListItem { Text = obj.ToString(), Value = ((int)obj).ToString() })
                    .AsEnumerable(), "Value", "Text");

                ViewBag.StatusList = new SelectList(Enum.GetValues(typeof(Web.Helpers.DataHelper.CarStatus))
                    .Cast<Web.Helpers.DataHelper.CarStatus>()
                    .Select(obj => new SelectListItem { Text = obj.ToString(), Value = ((int)obj).ToString() })
                    .AsEnumerable(), "Value", "Text");
        ...
        public ActionResult InsertCar(Car item)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    string _carImagePath = String.Concat(new DirectoryInfo(Server.MapPath("~")), "/", String.Format(functionHelper.GetFromConfig("CarImageFolder"), item.CarID));
                    if (!Directory.Exists(_carImagePath))
                        Directory.CreateDirectory(_carImagePath);
